...
if ! test -f $state/aura/scheme.json
    if command -v aura >/dev/null 2>&1
        aura scheme set -n shadotheme 2>/dev/null || log 'Could not set color scheme (aura command not ready)'
        sleep .5
        # Only reload if Hyprland is running
        if pgrep -x Hyprland >/dev/null 2>&1
            hyprctl reload 2>/dev/null || log 'Could not reload Hyprland'
        ...
        log 'Aura command not available yet, skipping scheme generation'
    end
end
...
if command -v aura >/dev/null 2>&1
    ...
    aura shell -d >/dev/null 2>&1 || log 'Could not start aura shell daemon'
else
    log 'Aura command not available yet, skipping shell daemon start'
end
