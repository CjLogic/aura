...
aura-refresh-config uwsm/default
aura-refresh-config uwsm/env
aura-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, aura-launch-editor' ~/.config/hypr/bindings.conf
fi
