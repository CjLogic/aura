...
# Users can still opt-in to Cloudflare DNS using: aura-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-aura-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-aura-dns.network
  ...
  echo "To use Cloudflare DNS, run: aura-setup-dns Cloudflare"
fi
